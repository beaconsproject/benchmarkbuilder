...
test_that("check reserve polygon colnames error", {
  ...
  expect_error(seeds_reserve(catchments_sf = builder_catchments_sample, reserve_polygons = existing_reserves_sample, name_col = "reserve", areatarget_col = "y"),
               "Column 'y' not in table")
})

test_that("check intactness col error", {
  existing_reserves_sample$areatarget <- 100000
  expect_error(seeds_reserve(catchments_sf = builder_catchments_sample, reserve_polygons = existing_reserves_sample, name_col = "reserve", areatarget_col = "areatarget",
...
test_that("check basic usage", {
  existing_reserves_sample$areatarget <- 100000
  expect_snapshot(as.data.frame(seeds_reserve(catchments_sf = builder_catchments_sample, reserve_polygons = existing_reserves_sample, name_col = "reserve", areatarget_col = "areatarget")))
})
...
test_that("check table conversion", {
  ...
  expect_snapshot(as.data.frame(
    reserve_seeds_to_builder(
      seeds_reserve(catchments_sf = builder_catchments_sample, reserve_polygons = existing_reserves_sample, name_col = "reserve", areatarget_col = "areatarget")
    )
  ))
})
...
test_that("check reserve seeds colname error", {
  seedz <- seeds_reserve(catchments_sf = builder_catchments_sample, reserve_polygons = existing_reserves_sample, name_col = "reserve", areatarget_col = "Areatarget")
  names(seedz) <- c("Name", "areatarget", "CATCHNUM")
  expect_error(builder_reserve(catchments_sf = builder_catchments_sample, seeds = seedz, neighbours = nghbrs),
               "Column 'Areatarget' not in table"
               )
})
...
test_that("check neighbours colname error", {
  ...
  nghbrs2 <- nghbrs
  names(nghbrs2) <- c("CATCHNUM", "ngh", "key")
  expect_error(builder_reserve(catchments_sf = builder_catchments_sample, seeds = seedz, neighbours = nghbrs2),
               "Column 'neighbours' not in table"
               )
})
...
test_that("check catchments colname error", {
  ...
  builder_catchments_sample2 <- builder_catchments_sample
  names(builder_catchments_sample2)[names(builder_catchments_sample2) == "ORDER1"] <- "ORDER_1"
  expect_error(builder_reserve(catchments_sf = builder_catchments_sample2, seeds = seedz, neighbours = nghbrs),
               "Column 'ORDER1' not in table"
               )
})
...
test_that("test outdir", {
  ...
  unlink(file.path(tempdir(), "builder2"), recursive = TRUE)
  builder_out1 <- builder_reserve(catchments_sf = builder_catchments_sample, seeds = seedz, neighbours = nghbrs, out_dir = file.path(tempdir(), "builder2"), benchmark_level_intactness = 0.5, catchment_level_intactness = 0)
  builder_out2 <- builder_reserve(catchments_sf = builder_catchments_sample, seeds = seedz, neighbours = nghbrs, benchmark_level_intactness = 0.5, catchment_level_intactness = 0)
  expect_equal(builder_out1, builder_out2)
  unlink(file.path(tempdir(), "builder2"), recursive = TRUE)
})
...
test_that("test an out_dir with spaces", {
  ...
  expect_error(
    builder_reserve(catchments_sf = builder_catchments_sample, seeds = seedz, neighbours = nghbrs, out_dir = file.path(tempdir(), "builder 2")),
    "out_dir cannot have spaces"
    )
})
...
test_that("basic builder with defaults", {
  seedz <- seeds_reserve(catchments_sf = builder_catchments_sample, reserve_polygons = existing_reserves_sample, name_col = "reserve", areatarget_col = "Areatarget")
  builder_out <- builder_reserve(catchments_sf = builder_catchments_sample, seeds = seedz, neighbours = nghbrs, catchment_level_intactness = 0.5, benchmark_level_intactness = 0.5)
  expect_snapshot(builder_out)
})
...
test_that("test an empty returned table, should return NULL", {
  ...
  expect_warning(
    expect_equal(builder_reserve(catchments_sf = builder_catchments_sample, seeds = seedz, neighbours = nghbrs), NULL),
    "No benchmarks to return"
  )
})
